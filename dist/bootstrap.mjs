...
function validatePinRules(pin) {
    ...
    if (!/[A-Z]/.test(pin))
        return { valid: false, error: 'PIN must contain at least 1 capital letter' };
    if (!/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?`~]/.test(pin))
        return { valid: false, error: 'PIN must contain at least 1 special character' };
    return { valid: true };
}
    ...
    if (urlPath === '/api/pin/verify' && req.method === 'POST') {
        ...
        try {
            ...
            if (!verifyPin(pin, u.pin_hash)) {
                const attempts = (u.pin_failed_attempts || 0) + 1;
                const lockUntil = attempts >= PIN_MAX_ATTEMPTS ? new Date(Date.now() + PIN_LOCKOUT_MINUTES * 60000).toISOString() : null;
                await pool.query('UPDATE users SET pin_failed_attempts = $1, pin_locked_until = $2 WHERE email = $3', [attempts, lockUntil, email]);
                await pool.end();
